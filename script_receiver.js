...
$(function() {
  ...
  function step3(room) {
    room.on('open', () => {
      const roomName = $('#join-room').val();
      const dummyPeer = new Peer({key: window.__SKYWAY_KEY__,});
            dummyPeer.on('open',() => {
                const dummyRoom = dummyPeer.joinRoom('sfu_video_' + roomName,{mode: 'sfu'});
                dummyRoom.on('open',function(){
                    dummyRoom.close();
                    dummyPeer.destroy();
                    console.log('dummyRoom opend');
                });
